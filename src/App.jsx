...
function App() {
  ...
  function AddToCart(id) { 
    ...
    if(!cart) {
      ...
      setCart(prevCart =>   
        prevCart.map(each =>  
          each.id === exists.id ? { ...each, quantity: each.quantity + 1 } : each  
        )  
      );  
    };
 ...
 function DeleteFromCart(id) {
  cart.map(eachDel => {
    if(eachDel.id === id && eachDel.quantity > 1) {
      ...
      setCart(preCart => 
        preCart.filter(eachFil => eachFil.id !== id)
      );
 ...
 function newOrder() {
  ...
  return (
    <ProductContext.Provider
     value={[data, AddToCart,Increase, Decrease, CartQuan, totalCartItem, totalCartItemPrice,DeleteFromCart,confirmOrder,newOrder, cart]}
